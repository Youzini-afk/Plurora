...
| provider ambiguity | Explicit `provider_package_id` | Never prefer an first-party provider implicitly. |
...
- `plurora/agentic-forge-lab` provides scratch branches, candidates, comparison, promotion, and replay;
- third-party replacement fixtures check that first-party implementations receive no implicit priority.
...
- hardcode first-party Package IDs in the UI as preferred implementations;
