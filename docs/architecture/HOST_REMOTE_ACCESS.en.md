...
An ordinary device planning or executing managed resources needs `realization.plan` or `realization.apply`, respectively, together with exact Installation, Target, and Realization selectors. Low-level target/exec/port/proxy adapters do not inherit those scopes.
