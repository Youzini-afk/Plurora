...
- Realization execution: `host.target.*`, `host.exec.*`, `host.port.*`, and `host.proxy.*` remain HostAdmin/HostDev adapters; the old `deploy` device scope is removed. `host.realization.*` uses OperationalIntent, TargetInventory, and stable plan digests for OCI/Docker build, Target-Agent apply/observe/stop, loopback ports, and proxy routes. Local and enrolled Agents share one typed operation/receipt path. Old deploy/build-deploy/Installation-deployment/Development-deployment routes are retired with no aliases. Rollback reads only persisted plans, never a live workspace.
