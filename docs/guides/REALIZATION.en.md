...
- Private events are not in the platform event registry and are unavailable through public journal/SSE.
...
## Appendix: executor and proxy

`host.exec.*`, `host.port.*`, and `host.proxy.*` are HostAdmin/HostDev adapters. Ordinary devices call only `host.realization.*`.

The local Host and a remote Target Agent receive the same typed operations (build / apply / observe / stop). Docker is the first backend: input must be an OCI image with `@sha256:` or a persisted build context. Docker is not a required Work concept.

The executor creates a Host-owned loopback port lease per endpoint and registers an HTTP route bound to that lease. Routes are Host-authenticated by default and become public only when the plan selects that policy. Stop cleans recorded resources only; it does not scan a live workspace.

Public errors return a stable code and next step. Architectural invariants are in [`../architecture/REALIZATION_CONTROLLER.md`](../architecture/REALIZATION_CONTROLLER.en.md) and [`../architecture/TARGET_AGENT_PROTOCOL.md`](../architecture/TARGET_AGENT_PROTOCOL.en.md).
