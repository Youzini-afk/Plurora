...
可接受来源：显式 `work.yaml` / `assembly.yaml`、Package manifest（单节点 Assembly）、普通源码仓库（先成为 Workspace / inspection 候选）、Foreign Capsule、content-only bundle。多个同等 provider 会报歧义，不按 publisher 自动选择。
