...
Accepted sources: explicit `work.yaml` / `assembly.yaml`, a Package manifest (single-node Assembly), an ordinary source repository (a Workspace / inspection candidate first), a Foreign Capsule, or a content-only bundle. Multiple equivalent providers are reported as ambiguous; the resolver never auto-picks by publisher.
