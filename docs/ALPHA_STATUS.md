...
- **Conformance：** 482 个具名 CLI conformance 外加 crate / service 单元测试持续通过；214 个 v1 schema（99 methods + 76 events + 39 top-level）通过校验；其中 7 个具名 case 覆盖 `modular-simulation`。
...
- `crates/plurora-cli/src/main.rs` 是薄入口。CLI 类型在 `cli.rs`，命令在 `commands/`，包模板在 `templates/`。conformance runner 与 case registry 已拆分：`conformance/runner.rs` 负责 `--list`、`--case`、`--tag`、`--fail-fast`、`--slowest`，`conformance/registry/` 按域注册 482 个 `ConformanceCase { id, tags, run }`。
...
`cargo run -p plurora-cli -- conformance` 跑 482 个具名 CLI 用例。支持：
