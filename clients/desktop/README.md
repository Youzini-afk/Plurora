...
health check, keeping RPC, SSE, Work surfaces, and proxies same-origin.
...
- configures a profile-relative SQLite event store (`desktop-events.sqlite`) so sessions, runtime projections, Realization jobs and revisions survive Desktop restarts;
- copies bundled first-party Manifests into `<data>/desktop/plurora-packages/` and autoloads git, integrity, install, Docker runtime, source intake, secret store, and workspace labs in dependency-safe order;
...
Existing custom autoload entries are preserved when the generated profile is refreshed. The bundled profile enables the package capabilities needed for Installation and Realization, but it does not enable unrestricted local execution or outbound access; those remain deny-all unless a user-owned profile explicitly grants them.

The managed loopback Host remains the default and continues serving the Desktop UI. From Settings → Host connections, the shared client can explicitly select an HTTPS remote Host; credentials and Installation/Target preferences are isolated per Host, and an unreachable or invalid remote can always fall back to the managed Host. Remote Work surfaces are sandboxed using the selected Host's `surface-frame.html`, so that Host must serve a matching Web static bundle when surface rendering is required.
